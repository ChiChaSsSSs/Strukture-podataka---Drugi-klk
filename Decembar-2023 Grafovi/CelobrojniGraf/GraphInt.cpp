...
int GraphAsListsInt::longestSimpleCycle()
{
	...
	{
		setStatusForAllNodes(0);
		longestForNode(ptr, ptr, 0, &maxCiklus);
		ptr = ptr->next;
	}
	return maxCiklus;
}

void GraphAsListsInt::longestForNode(LinkedNodeInt* start, LinkedNodeInt* tren, int trenutnaDuzina, int* maxDuzina)
{
	if (tren->status == 1)
	{
		if (tren == start)
	...
	{
		tren->status = 1;
		LinkedEdgeInt* tmpEdge = tren->adj;
		while (tmpEdge)
		{
			longestForNode(start, tmpEdge->dest, trenutnaDuzina + 1, maxDuzina);
