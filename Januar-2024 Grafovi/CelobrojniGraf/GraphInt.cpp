...
void GraphAsListsInt::setStatusForAllNodes(int status, int udaljenost) const
{
	LinkedNodeInt* ptr = start;
	while (ptr != nullptr) {
		ptr->status = status;
		ptr->udaljenost = udaljenost;
		ptr = ptr->next;
	}
}
...
bool GraphAsListsInt::arePathsPossible(LinkedNodeInt* start, LinkedNodeInt* goal1, LinkedNodeInt* goal2, LinkedNodeInt* mid)
{
	...
	stack.push(start);
	start->status = 2;
	int minDuzina = INT_MAX;
	while (!stack.isEmpty())
	{
		LinkedNodeInt* tmpNode = stack.pop();
		if (tmpNode != start)
			tmpNode->udaljenost = tmpNode->prev->udaljenost + 1;
		tmpNode->status = 3;

		LinkedEdgeInt* tmpEdge = tmpNode->adj;
		while (tmpEdge != NULL)
		{
			if (tmpEdge->dest != goal1 && tmpEdge->dest != goal2 && tmpEdge->dest->status == 1)
			{
				if (tmpEdge->dest != mid)
				{
					stack.push(tmpEdge->dest);
					tmpEdge->dest->status = 2;
					tmpEdge->dest->prev = tmpNode;
				}
				else
				{
					if (tmpNode->udaljenost + 1 < minDuzina)
					{
						minDuzina = tmpNode->udaljenost + 1;
						mid->prev = tmpNode;
						mid->udaljenost = tmpNode->udaljenost + 1;
					}
				}
			}
	...
	while (!stack.isEmpty())
	{
		...
		while (tmpEdge != NULL)
		{
			...
			{
				if (tmpEdge->dest == goal1)
					prvi = true;
				else if (tmpEdge->dest == goal2)
					drugi = true;
				if (prvi && drugi)
					return true;
				stack.push(tmpEdge->dest);
