...
void BSTreeInt::insert(int el, double vr)
{
	...
	if (root == nullptr)    // stablo je prazno
		root = new BSTNodeInt(el, vr);
	else if (par->isLT(el))
		par->right = new BSTNodeInt(el, vr);
	else
		par->left = new BSTNodeInt(el, vr);
	numOfElements++;
}
...
int BSTreeInt::brojCvorova(int keyMin, double valueMin)
{
	...
	bool veciPredak;
	if (root->vrednost > valueMin)
		veciPredak = true;
	else
		veciPredak = false;
	int pocetnaSuma = 0;

	ispitajZaJedanCvor(root->left, &pocetnaSuma, veciPredak, keyMin, valueMin);
	ispitajZaJedanCvor(root->right, &pocetnaSuma, veciPredak, keyMin, valueMin);

	cout << endl;
	return pocetnaSuma;
}

void BSTreeInt::ispitajZaJedanCvor(BSTNodeInt* koren, int* trenutnaVrednost, bool veciPredak, int keyMin, int valueMin)
{
	if (!koren || (koren->key <= keyMin && koren->right == NULL))
		return;

	bool trenutniUslov = veciPredak;
	if (!veciPredak)
	{
		if (koren->vrednost > valueMin)
			veciPredak = true;
	}
	ispitajZaJedanCvor(koren->left, trenutnaVrednost, veciPredak, keyMin, valueMin);
	ispitajZaJedanCvor(koren->right, trenutnaVrednost, veciPredak, keyMin, valueMin);
	
	if (trenutniUslov && koren->key > keyMin)
	{
