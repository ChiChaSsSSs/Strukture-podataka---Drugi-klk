...
BSTNodeInt* BSTreeInt::findNodeWithLargestRightSubtree()
{
	...
	int levo = izbrojiZaPodstablo(root, &max, &maxCvor);
	return maxCvor;
}

int BSTreeInt::izbrojiZaPodstablo(BSTNodeInt* root, int* max, BSTNodeInt** maxCvor)
{
	if (!root)
		return 0;
	
	izbrojiZaPodstablo(root->left, max, maxCvor);
	int trenutnaVrednost = izbrojiZaPodstablo(root->right, max, maxCvor);
	if (trenutnaVrednost > *max)
	{
		*max = trenutnaVrednost;
		*maxCvor = root;
	}
	return trenutnaVrednost + 1;
}
