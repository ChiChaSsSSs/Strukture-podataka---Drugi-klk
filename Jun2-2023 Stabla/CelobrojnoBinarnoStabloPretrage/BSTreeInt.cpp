...
BSTNodeInt* BSTreeInt::razlikaPodstabala(BSTNodeInt* root, BSTNodeInt** maksCvor, int* maksimum)
{
	if (root == NULL)
		return NULL;

	int levoPodstablo = izbrojiBrojCvorova(root->left);
	int desnoPodstablo = izbrojiBrojCvorova(root->right);

	int razlika = abs(levoPodstablo - desnoPodstablo);
	if (razlika > *maksimum)
	{
		*maksimum = razlika;
		*maksCvor = root;
	}
	razlikaPodstabala(root->left, maksCvor, maksimum);
	razlikaPodstabala(root->right, maksCvor, maksimum);
	return *maksCvor;
}

int BSTreeInt::izbrojiBrojCvorova(BSTNodeInt* root)
{
	if (root == NULL)
		return 0;

	return 1 + izbrojiBrojCvorova(root->left) + izbrojiBrojCvorova(root->right);
}
