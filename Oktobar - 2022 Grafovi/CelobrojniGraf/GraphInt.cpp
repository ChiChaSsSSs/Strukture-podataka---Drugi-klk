...
bool GraphAsListsInt::edgeExists(LinkedNodeInt* a, LinkedNodeInt* b)
{
	...
	while (tmpEdge != NULL && tmpEdge->dest != b)
...
bool GraphAsListsInt::isConnected()
{
	...
	if (brojObidjenih == nodeNum)
		return true;
	else
		return false;
}

void GraphAsListsInt::connectGraph()
{
	LinkedNodeInt* ptr = start;
	LinkedNodeInt* tmpNode = ptr->next;
	LinkedEdgeInt* tmpEdge = ptr->adj;
	while (!this->isConnected())
	{
		if (!edgeExists(ptr, tmpNode))
		{
			insertEdge(ptr->node, tmpNode->node);
			insertEdge(tmpNode->node, ptr->node);
		}
