...
int BSTreeInt::startFunc()
{
	if (!root)
		return 0;

	return countCloseSibilings(root);
}

int BSTreeInt::countCloseSibilings(BSTNodeInt* root)
{
	if (!root || (root->left == NULL && root->right == NULL))
		return 0;

	int rez = countCloseSibilings(root->left) + countCloseSibilings(root->right);
	if (root->left && abs(root->key - root->left->key) == 1)
		rez++;
