...
bool GraphAsListsInt::canReach(int a, int b, int maxNodes)
{
	LinkedNodeInt* startNode = NULL;
	LinkedNodeInt* endNode = NULL;
	LinkedNodeInt* ptr = start;
	setStatusForAllNodes(0);

	while (ptr != NULL && (startNode == NULL || endNode == NULL))
	{
		if (ptr->node == a)
			startNode = ptr;
		else if (ptr->node == b)
	...
	return canReachNodes(startNode, endNode, maxNodes, 1);
}

bool GraphAsListsInt::canReachNodes(LinkedNodeInt* startNode, LinkedNodeInt* endNode, int maxNodes, int trenutnaDuzina)
{
	if (startNode == endNode)
	{
		if (trenutnaDuzina <= maxNodes)
			return true;
		else
		{
			startNode->status = 0;
	...
	startNode->status = 1;
	LinkedEdgeInt* tmpEdge = startNode->adj;
	while (tmpEdge)
	{
		if (tmpEdge->dest->status != 1)
			if (canReachNodes(tmpEdge->dest, endNode, maxNodes, trenutnaDuzina + 1))
				return true;
