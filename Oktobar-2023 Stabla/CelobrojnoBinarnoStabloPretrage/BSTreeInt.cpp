...
BSTNodeInt* BSTreeInt::findDeepestLeaf()
{
	...
	BSTNodeInt* maksCvor = NULL;
	int maksDubina = 0;
	izracunajDubinu(root->left, &maksCvor, &maksDubina, 1);
	izracunajDubinu(root->right, &maksCvor, &maksDubina, 1);
	return maksCvor;
}

void BSTreeInt::izracunajDubinu(BSTNodeInt* koren, BSTNodeInt** maksCvor, int* maksDubina, int trenutnaDubina)
{
	if (!koren)
		return;

	if (koren->left == NULL && koren->right == NULL && trenutnaDubina > *maksDubina)
	{
		*maksDubina = trenutnaDubina;
		*maksCvor = koren;
		return;
	}

	izracunajDubinu(koren->left, maksCvor, maksDubina, trenutnaDubina + 1);
	izracunajDubinu(koren->right, maksCvor, maksDubina, trenutnaDubina + 1);
}
