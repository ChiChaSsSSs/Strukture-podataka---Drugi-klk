...
bool GraphAsListsInt::pathExists(LinkedNodeInt* a, LinkedNodeInt* b, LinkedNodeInt* c, LinkedNodeInt* d)
{
	...
	while (tmpEdge)
	{
		setStatusForAllNodes(0);
		a->status = 1;
		if (pronadjiSvePuteve(tmpEdge->dest, b, c, d, pocetak, false))
			return true;
...
bool GraphAsListsInt::pronadjiSvePuteve(LinkedNodeInt* start, LinkedNodeInt* end, LinkedNodeInt* node1, LinkedNodeInt* node2, bool nadjenPrvi, bool nadjenDrugi)
{
	if (!start)
		return false;
	if (start == end)
		if (nadjenPrvi && nadjenDrugi)
			return true;
		else
			return false;

	start->status = 1;
	LinkedEdgeInt* tmpEdge = start->adj;
	if (!nadjenPrvi)
	{
		if (start == node1)
		{
			nadjenPrvi = true;
			while (tmpEdge && tmpEdge->dest != node2)
				tmpEdge = tmpEdge->link;
			if (!tmpEdge)
				return false;
			return pronadjiSvePuteve(tmpEdge->dest, end, node1, node2, nadjenPrvi, nadjenDrugi);
		}
		else
		{
			while (tmpEdge)
			{
				if (tmpEdge->dest->status == 0 && tmpEdge->dest != node2 && tmpEdge->dest != end)
					return pronadjiSvePuteve(tmpEdge->dest, end, node1, node2, nadjenPrvi, nadjenDrugi);
				tmpEdge->dest->status = 0;
				tmpEdge = tmpEdge->link;
			}
		}
	}
	else
	{
		...
		while (tmpEdge)
		{
			if (tmpEdge->dest->status == 0)
				if (pronadjiSvePuteve(tmpEdge->dest, end, node1, node2, nadjenPrvi, nadjenDrugi))
					return true;
			tmpEdge->dest->status = 0;
			tmpEdge = tmpEdge->link;
		}
