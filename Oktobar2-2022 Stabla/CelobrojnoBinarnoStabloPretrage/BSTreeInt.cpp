...
BSTNodeInt* BSTreeInt::findSibiling(BSTNodeInt* p)
{
	if (!root || !p || root == p)
		return NULL;
	...
	BSTNodeInt* result = pronadjiCvorNaDubini(root->left, p, dubinaP, 1);
	if (result)
		return result;
	result = pronadjiCvorNaDubini(root->right, p, dubinaP, 1);
	return result;
}

BSTNodeInt* BSTreeInt::pronadjiCvorNaDubini(BSTNodeInt* koren, BSTNodeInt* ciljniCvor, int ciljnaDubina, int trenutnaDubina)
{
	if (!koren || trenutnaDubina > ciljnaDubina)
		return NULL;

	if (trenutnaDubina == ciljnaDubina && koren != ciljniCvor)
		return koren;

	BSTNodeInt* result = pronadjiCvorNaDubini(koren->left, ciljniCvor, ciljnaDubina, trenutnaDubina + 1);
	if (result)
		return result;
	result = pronadjiCvorNaDubini(koren->right, ciljniCvor, ciljnaDubina, trenutnaDubina + 1);
	return result;
}
